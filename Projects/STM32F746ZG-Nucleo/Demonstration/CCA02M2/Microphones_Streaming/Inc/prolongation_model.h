...
#ifndef __AI_PROLONGATION_MODEL_H__
#define __AI_PROLONGATION_MODEL_H__
#pragma once

#include "ai_platform.h"

#define AI_PROLONGATION_MODEL_MODEL_NAME          "prolongation_model"

#define AI_PROLONGATION_MODEL_IN_NUM       (1)
#define AI_PROLONGATION_MODEL_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 2, 44, 1, 1, NULL), \
}
#define AI_PROLONGATION_MODEL_IN_SIZE { \
  (2 * 44 * 1), \
}
#define AI_PROLONGATION_MODEL_IN_1_SIZE  (2 * 44 * 1)
#define AI_PROLONGATION_MODEL_IN_1_SIZE_BYTES  ((2 * 44 * 1) * 4)




#define AI_PROLONGATION_MODEL_OUT_NUM      (1)
#define AI_PROLONGATION_MODEL_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_PROLONGATION_MODEL_OUT_SIZE { \
  (1 * 1 * 1), \
}
#define AI_PROLONGATION_MODEL_OUT_1_SIZE  (1 * 1 * 1)
#define AI_PROLONGATION_MODEL_OUT_1_SIZE_BYTES  ((1 * 1 * 1) * 4)


AI_API_DECLARE_BEGIN

/*!
 * @defgroup prolongation_model
 ...
 * @ref ai_prolongation_model_create. Both the activations memory buffer 
 ...
 * using @ref ai_prolongation_model_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_PROLONGATION_MODEL_*
 * defines into file @ref prolongation_model.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_prolongation_model_forward(
  ai_handle network, const ai_buffer* input);


AI_API_DECLARE_END

#endif /*__AI_PROLONGATION_MODEL_H__*/
